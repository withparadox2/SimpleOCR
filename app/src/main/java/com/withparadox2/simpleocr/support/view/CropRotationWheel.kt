...
class CropRotationWheel(context: Context, attributeSet: AttributeSet) : FrameLayout(context, attributeSet) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        tempRect.top = (height - dp2px(22)) / 2f
        tempRect.right = (width + dp2px(2.5f)) / 2f
        tempRect.bottom = (height + dp2px(22)) / 2f
        canvas.drawRoundRect(tempRect, dp2px(2f), dp2px(2f), colorPaint)
    }

    private fun drawLine(canvas: Canvas, i: Int, delta: Float, width: Int, height: Int, center: Boolean, paint: Paint) {
        var drawPaint = paint
        val radius = (width / 2.0f - dp2px(70)).toInt()

        val angle = 90 - (i * DELTA_ANGLE + delta)
        val offset = (radius * Math.cos(Math.toRadians(angle.toDouble()))).toInt()
        val x = width / 2 + offset
        ...
        canvas.drawRect((x - w / 2).toFloat(), ((height - h) / 2).toFloat(), (x + w / 2).toFloat(), ((height + h) / 2).toFloat(), drawPaint)
    }
